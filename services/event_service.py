...
def create_event():
    ...
    with SessionLocal as session:
        try:
            ...
            notes = Prompt.ask(
                "[bold yellow]Notes (optional)[/bold yellow]", default="")
            ...
            sentry_sdk.capture_message(
                f"Event '{event_name}' created successfully!",
                level="info"
            )
        except Exception as e:
            session.rollback()
            console.print(Panel(f"[bold red]Error creating event: {e}[/bold red]",
                                box=box.ROUNDED))
    ...
    with SessionLocal as session:
        try:
            ...
            end_date_str = Prompt.ask(
                ...
                f" [bold green](current: {
                    event.event_end_date.strftime('%d-%m-%Y %H:%M')})"
                "[/bold green]",
            ...
            if update_support_contact == 'Y':
                ...
                for emp in support_employees:
                    console.print(f"ID: [cyan]{emp.employee_id}[/cyan], Name: [green]{
            ...
            # Log success message to Sentry
            sentry_sdk.capture_message(
                f"Event '{event_name}' updated successfully!",
                level="info"
            )
        except ValueError as ve:
            console.print(
            ...
            session.rollback()
            console.print(Panel(f"[bold red]Error updating event: {e}[/bold red]",
                                box=box.ROUNDED))
            # Log unexpected exception to Sentry
            sentry_sdk.capture_exception(e)
